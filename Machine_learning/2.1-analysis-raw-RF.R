...
## TOX-Project: Analyze of unprocess data. Analyze included: 
##     > log transformation and data visualization
...
boxplot(
    ...
    col=spl.cols,
    las=2,
    main="Boxplot of observations - log(Raw data)",
    cex.axis=.5
)
...
for(idx in 1:length(foldss)){
    ...
    ## Remove HDA and NAC as factors
    dF = data.frame(
            metadata[,c("Experiment","Cellline","Section","Condition")],
            exprs
    )
    dF[!sapply(dF, is.numeric)] = lapply(dF[!sapply(dF, is.numeric)],as.factor)
    ...
    rF.list = mclapply(1:length(folds),function(i){
            ...
            list(
                ...
                importance = rF$importance[
                    order(rF$importance, decreasing = T),
                ],
                confusion = table(Obs = dF$Condition[test], pred = pred)
            )
        },
...
for(i in 1:nrow(df)) lines(
    c(x[i],x[i]),with(df,c(Accuracy[i]-sd[i],Accuracy[i]+sd[i])))
for(i in 1:nrow(df)) lines(
    c(x[i]-.1,x[i] + .1),rep(with(df,c(Accuracy[i]-sd[i])),2))
for(i in 1:nrow(df)) lines(
    c(x[i]-.1,x[i] + .1),rep(with(df,c(Accuracy[i]+sd[i])),2))
axis(1,x,df$Run,las=2)
rm(x,df)
...
rownames(imports)[rownames(imports) %in% names(feature.names)]  = 
    feature.names[
        rownames(imports)[
            rownames(imports) %in% names(feature.names)
        ]
    ]
