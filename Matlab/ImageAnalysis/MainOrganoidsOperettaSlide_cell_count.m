...
% allows scrolling through the different planes in the stitched image.
   ...
   ObjectsAll = {};
    ...
    PathMAP = cellfun(@(x) ~isempty(x), strfind(FileThis.name, 'MAP'));
    chMAP = load([FileThis{PathMAP, 'folder'}{:}, filesep, FileThis{PathMAP, 'name'}{:}]); chMAP = chMAP.MAP2647;
    ...
    PathTuj1 = cellfun(@(x) ~isempty(x), strfind(FileThis.name, 'TUJ'));
    chTuj1 = load([FileThis{PathTuj1, 'folder'}{:}, filesep, FileThis{PathTuj1, 'name'}{:}]); chTuj1 = chTuj1.TUJ1568;
       
%% Image analysis 
        Label = Groups(g);
        try
            ObjectsThisOrganoid = f_ImageAnalysisPerOperettaOrganoid_cell_count(Label, chTH, chMAP, chHoechst, chTuj1, PreviewSavePath);
            catch
